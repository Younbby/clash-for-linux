...
Clash Dashboard 访问地址：http://<ip>:7896/ui
...
```bash
$ netstat -tln | grep -E '7896|789.'
tcp        0      0 127.0.0.1:7896          0.0.0.0:*               LISTEN     
tcp6       0      0 :::7893                 :::*                    LISTEN     
tcp6       0      0 :::7894                 :::*                    LISTEN     
tcp6       0      0 :::7895                 :::*                    LISTEN
```
...
通过浏览器访问 `start.sh` 执行成功后输出的地址，例如：http://192.168.0.1:7896/ui

- 登录管理界面

在`API Base URL`一栏中输入：http://\<ip\>:7896 ，在`Secret(optional)`一栏中输入启动成功后输出的Secret。
